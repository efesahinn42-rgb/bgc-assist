...
export default function AgencyApplicationsPage() {
    ...
    const [detailModal, setDetailModal] = useState<AgencyApplication | null>(null);
    ...
    const handleDelete = async (id: string) => {
        try {
            const res = await fetch(`/api/agency-applications/${id}`, { method: "DELETE" });
            if (res.ok) {
                setDeleteModal(null);
                fetchApplications();
            }
    ...
    const updateStatus = async (id: string, newStatus: string) => {
        try {
            const res = await fetch(`/api/agency-applications/${id}`, {
                method: "PUT",
    ...
    return (
        <div className="min-h-screen">
            <AdminHeader
                title="Acente Başvuruları"
                description="Acente başvurularını yönetin"
            />
                        ...
                        <table className="w-full min-w-[800px]">
                            <thead className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Yetkili</th>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        ...
                                                        {app.taxNumber && (
                                                            <p className="text-xs text-gray-500">VN: {app.taxNumber}</p>
                                                        )}
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <p className="text-gray-900 dark:text-white">{app.authorizedName}</p>
                                            </td>
                                            <td className="px-6 py-4">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    ...
                    >
                        ...
                        <div className="space-y-4">
                            ...
                            {detailModal.taxNumber && (
                                <div>
                                    <label className="text-sm text-gray-500">Vergi Numarası</label>
                                    <p className="font-medium text-gray-900 dark:text-white">{detailModal.taxNumber}</p>
                                </div>
                            )}
                            <div>
